...
import { MediaGallery } from "./drawer/MediaGallery"
import { RatingStars, ReviewCard, DetailSection, MetricCard, DrawerTab, AskPrompt } from "./drawer/DrawerPrimitives"
...
function isGenericItineraryVenue(value: string) {
  ...
  return [
    "nearby bistro", "nearby cafe", "nearby restaurant", "market restaurant",
    "local restaurant", "local cafe", "restaurant", "cafe", "bistro",
  ].includes(normalized)
}
          ...
          >
            ...
            >
              ...
              {place?.rating != null && (
                <div className="flex flex-wrap items-center gap-2.5 mb-3">
                  ...
                  {primaryReviewCount ? (
                    <span className="text-sm" style={{ color: "var(--text-muted)" }}>· {primaryReviewCount.toLocaleString()} reviews</span>
                  ) : null}
                </div>
              )}
                ...
                <div className="mb-3 space-y-1.5">
                  ...
                  {hotel.review_score != null && (
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ background: "var(--accent)", color: "white" }}>
                        {hotel.review_score}
                      </span>
                      {hotel.review_word && (
              ...
              <DetailSection id="drawer-section-traveler" eyebrow="Traveler Signal" title="What Tripadvisor adds here">
                ...
                {tripadvisorLoading ? (
                  ...
                  <div className="space-y-2.5">
                    ...
                    {tripadvisor.details.ranking_string && (
                      <p className="text-sm font-semibold" style={{ color: "var(--text)" }}>{tripadvisor.details.ranking_string}</p>
                    )}
                    ...
                    {hotel && (tripadvisor.details.photo_count || tripadvisor.details.see_all_photos) && (
                      <div className="flex flex-wrap items-center gap-3 text-xs" style={{ color: "var(--text-muted)" }}>
                        ...
                        {tripadvisor.details.see_all_photos && (
                          <a href={tripadvisor.details.see_all_photos} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 font-semibold" style={{ color: "#f59e0b" }}>
                    ...
                    {tripadvisor.details.web_url && (
                      <a href={tripadvisor.details.web_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-semibold" style={{ color: "#f59e0b" }}>
            ...
            {activeTab === "overview" && detailSummary && (
              <DetailSection id="drawer-section-overview" eyebrow="Overview" title="Why this place is worth opening">
                <p className="text-sm leading-relaxed" style={{ color: "var(--text-muted)" }}>{detailSummary}</p>
              </DetailSection>
            )}
            ...
            {activeTab === "overview" && eventDetail && !place && !hotel && (
              <DetailSection eyebrow="Fallback Detail" title="Itinerary stop">
                <div className="space-y-3">
                  {eventLookupLoading && (
                    <p className="text-xs" style={{ color: "var(--accent-light)" }}>Looking up live place details…</p>
                  )}
                  ...
                  {eventDetail.subtitle && (
                    <p className="text-sm leading-relaxed" style={{ color: "var(--text-muted)" }}>{eventDetail.subtitle}</p>
                  )}
                  {eventDetail.price_local && (
                    <p className="text-sm font-semibold" style={{ color: "var(--text)" }}>{eventDetail.price_local}</p>
                  )}
                </div>
              </DetailSection>
            )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {centerCoordinates ? (
                      <button
                        ...
                        style={{ background: "rgba(61,140,214,0.14)", color: "var(--accent-light)", border: "1px solid rgba(61,140,214,0.3)" }}
                      >
                        Center on map
                      </button>
                    ) : null}
                  ...
                  {tripadvisor.details.reviews.slice(0, 2).map((review, index) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 mb-1.5">
                        ...
                        {review.rating != null && (
                          <span className="text-xs font-semibold" style={{ color: "#f59e0b" }}>★ {review.rating}</span>
                        )}
                      </div>
                      {review.title && (
                        <p className="text-xs font-semibold mb-1" style={{ color: "var(--text)" }}>{review.title}</p>
                      )}
                      {review.text && (
                        <p className="text-xs leading-relaxed" style={{ color: "var(--text-muted)" }}>{review.text}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {activeTab === "reviews" && place?.reviews && place.reviews.length > 0 && (
              <DetailSection
                id={tripadvisor?.matched && tripadvisor.details?.reviews?.length ? undefined : "drawer-section-reviews"}
                eyebrow="Reviews"
